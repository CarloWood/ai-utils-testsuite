...
int main()
{
  ...
  assert(p1->unique() && p1->count() == 1);

  boost::intrusive_ptr<Test> p2 = p1;
  {
    boost::intrusive_ptr<Test> p3(p1);
    assert(!p1->unique() && p3->count() == 3);
    p2.reset();
    assert(!p1->unique() && p3->count() == 2);
  }
  assert(p1->unique() && p1->count() == 1);
  ...
  std::swap(p2, q2);
  assert(p1->count() == 3);
  assert(p2->count() == 2);     // swapped with q2
  assert(p3->count() == 3);
  assert(q1->count() == 2);
  assert(q2->count() == 3);     // swapped with p2
  ...
  p2 = q2;
  assert(p1->count() == 4);
  assert(p2->count() == 4);     // same as q2
  assert(p3->count() == 4);
  assert(q1->count() == 1);     // decremented with 1.
  assert(q2->count() == 4);     // incremented with 1.
